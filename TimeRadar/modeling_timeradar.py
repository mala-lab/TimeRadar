...
from .configuration_timeradar import TimeRadarConfig
...
@dataclass
class TimeRadarOutput(ModelOutput):
    """Structured outputs returned by TimeRadar anomaly detection."""
...
class TimeRadarForAnomalyDetection(PreTrainedModel):
    """Hugging Face wrapper around the original pretrained TimeRadar network."""

    config_class = TimeRadarConfig
    base_model_prefix = "model"
    main_input_name = "input_values"

    # Build the original network and preserve the three training-time buffers.
    def __init__(self, config: TimeRadarConfig):
        super().__init__(config)
        self.model = Model(config)
        self.model.register_buffer("tau_n", torch.tensor(0.0))
        self.model.register_buffer("tau_a", torch.tensor(0.0))
        self.model.register_buffer("tau_inited", torch.tensor(False))
        self.post_init()

    # Validate fixed-window constraints before running the pretrained network.
    def _validate_inputs(self, input_values, copies, mask_mode):
        if input_values.ndim != 3:
            raise ValueError(
                "input_values must have shape [batch, sequence_length, channels]"
            )
        if input_values.shape[1] != self.config.seq_len:
            raise ValueError(
                f"Expected sequence length {self.config.seq_len}, "
        ...
        if mask_mode not in {"c", "random", "nomask"}:
            raise ValueError(f"Unsupported mask_mode: {mask_mode}")

    # Run reconstruction and return per-timestamp anomaly scores.
    def forward(
        self,
        ...
        anomaly_score_weight: Optional[float] = None,
        return_dict: Optional[bool] = None,
    ):
        copies = self.config.copies if copies is None else copies
        norm = self.config.norm if norm is None else norm
        mask_mode = self.config.mask_mode if mask_mode is None else mask_mode
        score_weight = (
            self.config.anomaly_score_weight
            if anomaly_score_weight is None
            else anomaly_score_weight
        )
        return_dict = self.config.use_return_dict if return_dict is None else return_dict

        if not 0.0 <= score_weight <= 1.0:
            raise ValueError("anomaly_score_weight must be between 0 and 1")
        self._validate_inputs(input_values, copies, mask_mode)

        representations, reconstructions = self.model(
            input_values,
            norm=int(norm),
            mask_mode=mask_mode,
            copies=copies,
        ...
        variance_score = torch.var(reconstructions, dim=0).mean(dim=-1)
        mean_reconstruction = reconstructions.mean(dim=0)
        reconstruction_score = ((mean_reconstruction - input_values) ** 2).mean(dim=-1)
        anomaly_scores = (
            score_weight * variance_score
            + (1.0 - score_weight) * reconstruction_score
        )

        if not return_dict:
            return anomaly_scores, reconstructions, representations
        return TimeRadarOutput(
