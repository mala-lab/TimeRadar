...
class Model(nn.Module):
    ...
    # ?ablation study random patch masking
    def adaptive_energy_mask(self, z):
        ...
        flat_energy = energy.view(bs, -1)  # Flattening H and W into a single dimension
        median_energy = flat_energy.median(dim=1, keepdim=True)[0]  # Compute median
        median_energy = median_energy.view(bs, 1)  # Reshape to match the original dimensions
    ...
    def decomposed_frequency_learning(self, x):
        ...
        # x = x.reshape(bs * n_vars, patch_len, patch_num)                        # [bs * n_vars x patch_len x patch_num]
        ...
        # # ?画图
        # z_main_np = torch.abs(z_main).pow(2).sum(dim=-1)
        # z_main_np = z_main_np.detach().cpu().numpy()
        # np.save('tools/main_freq.npy', z_main_np)
        # res energy part
        z_res = z - z_main
        # ?画图
        # z_res_np = torch.abs(z_res).pow(2).sum(dim=-1)
        # z_res_np = z_res_np.detach().cpu().numpy()
        # np.save('tools/res_freq.npy', z_res_np)
        ...
        # x = x.reshape(bs * self.freq_num, patch_num, n_vars, patch_len)         # [bs * freq_num x patch_num x nvars x patch_len]
        return x
    ...
    def forward(self, x_enc):
        ...
        x = self.decomposed_frequency_learning(x)                               # [freq_num x bs x seq_len x nvars]
        ...
        # patch embedding
        x = self.patch_embed(x)                                                 # [bs * freq_num x patch_num x nvars x d_model] 

        # sharedn backbone
        # z = self.backbone(z)
        x = x.transpose(1, 2)                                                    # z: [bs * freq_num x nvars x patch_num x d_model]        
        u = torch.reshape(x, (-1, n_vars * patch_num, self.d_model))             # u: [bs' x nvars * patch_num x d_model]
        u = self.dropout(u + self.W_pos)                                         # u: [bs' x nvars * patch_num x d_model]


        x = self.encoder(u)                                                      # x: [bs' x d_model x nvars * patch_num]
        x = torch.reshape(x, (-1, n_vars, patch_num, self.d_model))              # x: [bs' x nvars x patch_num x d_model]
        x = x.permute(0, 1, 3, 2)                                                # x: [bs' x nvars x d_model x patch_num]

        # multi-scale frequency aggregation
        x = x.view(-1, bs, n_vars, self.d_model, patch_num).mean(dim=0)         # x: [bs x nvars x d_model x patch_num] 
        x_vec = x
